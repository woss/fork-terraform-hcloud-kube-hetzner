...
* **Node Annotations Are Create-Only:** `control_plane_nodepools[*].annotations`, `agent_nodepools[*].annotations` (plus `nodes[*].annotations` overrides), and `autoscaler_nodepools[*].annotations` render into node-local cloud-init. A systemd oneshot runs after the local k3s/RKE2 node service becomes active and patches only its own Kubernetes Node with the kubelet kubeconfig. These annotations are applied when a node joins; later Terraform map changes affect only new/replaced nodes and do not reconcile removals on existing Nodes. Use this for creation-time defaults such as Longhorn `node.longhorn.io/default-disks-config`, not as an ongoing annotation controller.
...
* **Cilium Multinetwork Preview Mode:** `multinetwork_mode = "cilium_public_overlay"` is a lab-only v3 preview for spanning multiple Hetzner Cloud Networks. It requires `enable_experimental_cilium_public_overlay = true`, `cni_plugin = "cilium"`, public node addresses for `multinetwork_transport_ip_family` (`ipv4`, `ipv6`, or `dualstack`), and a public control-plane endpoint or public control-plane load balancer. When IPv6 or dual-stack cluster CIDRs are enabled, the overlay transport family must include IPv6 so kubelet `node-ip` matches the cluster CIDR families. The module forces Cilium tunnel mode with WireGuard/node encryption, opens Cilium peer ports, disables Hetzner CCM route reconciliation, avoids control-plane fanout to every external agent network, uses public load-balancer targets, and renders one cluster-autoscaler Deployment per effective `network_id`. This path is not production-supported until live cross-network Cilium datapath validation passes. Raw Flannel/Calico private multinetwork scale and Robot/vSwitch route exposure as a Cloud-only scale solution remain unsupported.
...
* **v3 Support Levels:** k3s on Leap Micro is the stable default. RKE2 on Leap Micro, OpenTofu, Cilium Gateway API, and embedded registry mirror are supported opt-in where their validation rules pass. Cilium dual-stack for static HCloud control-plane/agent nodes is plan-validated but live E2E remains pending; use the default Cilium tunnel mode, because native IPv6 pod routing is rejected in this release. Standard private-network IPv6-only is rejected because direct node-to-node public IPv6 firewall paths are not opened by the module in this release. Cilium IPv6/dual-stack with active autoscaler nodepools currently requires the experimental public-overlay path because standard autoscaler cloud-init does not yet derive IPv6/dual-stack `node-ip` values at boot. `nat_router`, `extra_robot_nodes`, and Tailscale node transport remain private IPv4-only paths and are rejected with IPv6 cluster CIDRs in this release. Tailscale node transport is supported opt-in for secure single-network clusters and Flannel-first private multinetwork scale-out without IPv6 pod/service CIDRs. Cilium public-overlay multinetwork is an experimental preview, not a production-supported v3 promise yet. MicroOS is legacy/upgrade support. Cloudflare Access/Tunnel and user-owned Tailscale/ZeroTier/WireGuard/WARP remain external access patterns through generic hooks when they are only for operator access, app access, or post-bootstrap features. Robot/vSwitch, private-only, Longhorn-heavy, and existing-network upgrades are advanced/special-case paths that deserve blue/green consideration. Raw Flannel/Calico multinetwork scale without Tailscale/routed transport is unsupported.
...
* **`ingress_controller` (String, Optional):**
  ...
  * **Options:**
    ...
    * `"nginx"`: Deploys the community [Ingress-NGINX controller](https://kubernetes.github.io/ingress-nginx/), not F5 NGINX Ingress Controller. Use `"none"` when installing F5 independently.
...
```terraform
  ...
  # Appended AFTER module-generated apiserver args (auth/audit); duplicate k3s/rke2 config flag keys
  # are last-wins, so this can intentionally override module defaults as an escape hatch.
  #
  # kube_apiserver_args = [
...
* **`kube_apiserver_args` (List of Strings, Optional, Default: `[]`):**
  ...
  * **Format:** A list of `"flag=value"` strings **without** the leading `--` (the same convention k3s/rke2 use inside `config.yaml`). Null, empty, whitespace-only, leading/trailing-whitespace, and `--flag=value` entries are rejected by input validations.
  * **How it applies:** The values are appended **after** the module's own apiserver flags (e.g. those derived from `authentication_config` / `audit_policy_config`) in the rendered `config.yaml`. k3s/rke2 duplicate config flag keys are last-wins, so user entries can intentionally override module defaults as an escape hatch. Changes apply **in-place** through the existing config-update provisioner — the k3s/rke2 service is restarted on each control-plane node, **no node is recreated**.
...
* **`cilium_routing_mode` (String, Optional, relevant if `cni_plugin = "cilium"`):**
  ...
  * **IPv6 Constraint:** `cilium_routing_mode = "native"` is rejected with `cluster_ipv6_cidr` / `service_ipv6_cidr` in this release; use the default tunnel mode for dual-stack.
...
* **`cluster_ipv6_cidr` / `service_ipv6_cidr` (String, Optional):**
  ...
  * **Considerations:** Static HCloud control-plane and agent nodes advertise `node-ip` values matching the configured cluster CIDR families: private IPv4 for IPv4 clusters and private IPv4 plus public IPv6 for dual-stack clusters. Do not enable dual-stack in-place on an existing IPv4-only k3s/RKE2 cluster; cluster/service CIDR families are bootstrap-time choices and require a new cluster or blue/green migration. Standard private-network IPv6-only is rejected in this release because direct node-to-node public IPv6 firewall paths are not opened by the module. `nat_router`, `extra_robot_nodes`, and `node_transport_mode = "tailscale"` are private IPv4-only paths and are rejected with IPv6 cluster CIDRs in this release. Cilium native routing is rejected with IPv6 cluster CIDRs; use the default tunnel mode. Active autoscaler nodepools are rejected for standard private-network IPv6/dual-stack clusters until autoscaler cloud-init can derive IPv6/dual-stack `node-ip` values at boot; use static HCloud agents or the experimental public-overlay path with a transport family that includes every enabled cluster CIDR family.
...
* **`extra_robot_nodes` (List Object, Optional):**
  ...
  * **IPv6 Constraint:** Robot nodes currently render an IPv4-only `node-ip` from `private_ipv4`, so `extra_robot_nodes` is rejected with `cluster_ipv6_cidr` / `service_ipv6_cidr` in this release.
...
* **`node_transport_mode` (String, Optional):**
  ...
  * **Requirements:** Tailscale mode requires `tailscale_node_transport.magicdns_domain`, restricted or closed public API/SSH firewall sources, no public module-managed control-plane Load Balancer, non-overlapping Tailnet/cluster/service/network CIDRs, no IPv6 pod/service CIDRs in this release, and `multinetwork_mode = "disabled"`.
